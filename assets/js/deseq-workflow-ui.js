(function () {
  const config = window.DESEQ_WORKFLOW_CONFIG || {};
  const state = {
    jobId: null,
    pollTimer: null,
    apiToken: "",
  ...
  async function loadSyntheticPreview() {
    try {
      ...
      setStatus("Synthetic toy dataset loaded for preview. You can submit it when the API is deployed.", "success");
    } catch (error) {
  ...
  async function submitJob(datasetMode) {
    if (!config.apiBaseUrl || config.demoMode) {
      setStatus("Demo mode is active. Deploy the Cloudflare Worker/API stack and set apiBaseUrl to submit live jobs.", "info");
      return;
    }
    ...
    const response = await fetch(`${config.apiBaseUrl}/tools/run_deseq`, {
      method: "POST",
      headers: {
  ...
  function bindEvents() {
    $("#use-synthetic")?.addEventListener("click", loadSyntheticPreview);
    $("#submit-synthetic")?.addEventListener("click", () => submitJob("synthetic").catch((error) => setStatus(error.message, "error")));
    $("#submit-upload")?.addEventListener("click", () => submitJob("uploaded").catch((error) => setStatus(error.message, "error")));
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    loadSampleManifest();
    if (!config.apiBaseUrl || config.demoMode) {
      setStatus("Demo mode: sample outputs are shown below. Deploy the backend to enable live submissions.", "info");
    }
  });
})();
