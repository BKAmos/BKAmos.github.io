"""Small Redis/RQ job helpers for contamination API."""
...
def make_job_id() -> str:
    return f"contam-{uuid.uuid4().hex[:12]}"
...
def enqueue_job(job_id: str, request_payload: dict[str, Any]) -> None:
    set_job(job_id, {"status": "queued", "request": request_payload, "status_url": f"/jobs/{job_id}"})
    queue().enqueue(
        "jobqueue.worker_loop.run_queued_investigation_job",
        job_id,
        request_payload,
