...
<#
  POST /tools/run_investigation using Invoke-RestMethod (avoids PowerShell/curl JSON quoting issues).
...
Write-Host "POST $BaseUrl/tools/run_investigation"
$resp = Invoke-RestMethod -Uri "$BaseUrl/tools/run_investigation" -Method Post -Headers $headers -Body $body
