...
<#
  Fire N concurrent POST /tools/run_investigation requests (synthetic dataset).
...
$payload.profile = "edge_case"
$body = $payload | ConvertTo-Json -Depth 20

Write-Host "Submitting $Count jobs to $BaseUrl/tools/run_investigation ..."
$jobs = @( foreach ($i in 1..$Count) {
        Start-Job -Name "deseq-$i" -ScriptBlock {
            ...
            try {
            Invoke-RestMethod -Uri "$url/tools/run_investigation" -Method Post -Headers $h -Body $json
            }
            catch {
