...
def volcano_plot(
    ...
    *,
    ...
    ax.set_xlabel(f"log2 fold change ({treatment_level} vs {reference_level})")
...
def ma_plot(
    ...
    *,
    ...
    ax.set_ylabel(f"log2 fold change ({treatment_level} vs {reference_level})")
    fig.tight_layout()
    fig.savefig(out_path, dpi=140)
    plt.close(fig)
...
def pca_plot(
    ...
    *,
    ...
    colors = {reference_level: "#1f77b4", treatment_level: "#d62728"}
    fig, ax = plt.subplots(figsize=(6.5, 5))
    for condition, group in plot_df.groupby(condition_column, observed=False):
        ax.scatter(
            group["PC1"],
            group["PC2"],
            label=condition,
    ...
    ax.legend(title=condition_column)
    fig.tight_layout()
    fig.savefig(out_path, dpi=140)
    plt.close(fig)
...
def top_genes_heatmap(
    ...
    *,
    ...
    sort_columns = [condition_column]
    if batch_column and batch_column in metadata.columns:
        sort_columns.append(batch_column)
    sort_columns.append("sample_id")
    sample_order = metadata.sort_values(sort_columns)["sample_id"].tolist()
